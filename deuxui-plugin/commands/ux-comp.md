...
Invoke the `deuxui` skill and read `references/ops/visualize.md`.
...
**The world is already committed.** Read `.deuxui/design.contract.yaml` first. If
...
```bash
python3 scripts/ux_image.py brief --write .deuxui/comps/$1.brief.yaml --surface "$1"
# edit the regions: `share` is the vertical proportion, `medium` is flat or plate
python3 scripts/ux_image.py render .deuxui/comps/$1.brief.yaml
```
...
python3 scripts/ux_image.py generate .deuxui/comps/$1.brief.yaml
python3 scripts/ux_image.py verify .deuxui/comps/$1-A.png
```
...
```bash
python3 scripts/ux_question.py ask .deuxui/comps/$1.comps.yaml
```
