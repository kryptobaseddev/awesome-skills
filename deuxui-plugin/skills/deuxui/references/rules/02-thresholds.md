...
Every number this skill enforces, with its class. STANDARD values come from WCAG and are not negotiable within their scope. PROJECT values are defaults you may override in `.deuxui/ux.config.yaml` with a recorded reason. Machine-readable form: `references/rules/thresholds.yaml`.
...
| **NUM-017** | P2 | PROJECT | Use the existing scale. For a new system, start with 4-unit spacing increments and named exceptions. | Check optical alignment and real content. Do not force every dimension onto the scale. | S00 | `S-TOKEN-ARBITRARY`, `S-CONTRACT-RAMP` |
