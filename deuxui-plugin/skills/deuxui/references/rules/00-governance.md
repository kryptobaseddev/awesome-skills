...
| **GOV-015** | P2 | PROJECT | MUST be able to resolve every recorded approval to the declaration it was made against, by archiving the contract's bytes under their own hash rather than recording only a hash. | Every decision record that approves a build names a contract_sha, and those bytes exist in the archive. An approval whose declaration cannot be produced is reported as unresolvable rather than read against the current contract. | S00 | `A-CONTRACT-ARCHIVED` |
