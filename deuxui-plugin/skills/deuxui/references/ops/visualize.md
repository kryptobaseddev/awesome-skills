...
python3 scripts/ux_image.py prompt .deuxui/comps/detail.brief.yaml --option A
python3 scripts/ux_image.py generate .deuxui/comps/detail.brief.yaml
```

Four providers, tried in order and each reporting its own absence: a harness's own
image tool via `DEUXUI_IMAGE_CMD` (a command template with `{prompt_file}`,
...
```bash
python3 scripts/ux_image.py verify .deuxui/comps/listing-detail-A.png
```
