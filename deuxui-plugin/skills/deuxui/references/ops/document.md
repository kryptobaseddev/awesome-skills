...
impeccable's `document` writes a DESIGN.md by reading the code. deuxui writes
...
python3 scripts/comp_spec.py .deuxui/reports/runtime/screens/root_fullpage.png
```
...
`.deuxui/DESIGN.md` carries what the contract cannot: which components exist and
...
And `.deuxui/PRODUCT.md`: audience, jobs to be done, the rare high-impact tasks
