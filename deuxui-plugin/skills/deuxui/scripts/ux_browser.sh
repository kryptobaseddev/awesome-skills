#!/usr/bin/env bash
# deuxui runtime tier -- measures a running interface instead of reading about it.
#
#   ux_browser.sh [base-url] [--routes /,/settings] [--api '**/api/**']
#                             [--out DIR] [--viewports 320,390,768,1024,1440]
#                             [--config PATH] [--baseline PNG]
#
# Every value defaults to app.* in .deuxui/ux.config.yaml; a flag overrides the
...
BASE=""; ROUTES=""; API=""; OUT=".deuxui/reports/runtime"; VIEWPORTS=""; CONFIG=""
...
if [ -z "$BASE" ]; then
  ...
  echo "  no base URL given and no app.dev_url in .deuxui/ux.config.yaml" >&2
  exit 1
fi
if [ -z "$API" ]; then
