...
Derived from impeccable's `hooks` (Apache-2.0); the contract below is deluxui's
...
The deluxui plugin ships this wired. Standalone:
...
```json
{
  "hooks": {
    "PostToolUse": [{
      ...
      "hooks": [{
        "type": "command",
        "command": "python3 /path/to/deluxui/scripts/ux_check.py --stdin"
      }]
    }]
  }
}
