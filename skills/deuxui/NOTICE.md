...
Parts of deuxui's design layer are derived from the **impeccable** skill by Paul Bakaus.
...
agent to follow, while deuxui has to survive being checked, so a claim that could not be
bound to a detector or a declaration was either converted or left out.

| From impeccable | In deuxui | Change |
|---|---|---|
...
| The `live` review loop's first half -- selecting an element in the running interface and saying what is wrong with it | `scripts/ux_review.py`, `scripts/ux_select.py`, `references/ops/prototype.md` | Rebuilt on two channels deuxui already had: an HTTP proxy that hosts both variants same-origin, and the Chrome DevTools Protocol for an app the reviewer already has open. No dev-server plugin and no framework adapter, because nothing is being written back into the page. Each note carries the element's selector, its own text, its computed type, colour, spacing, radius, shadow, box and viewport, so it is a work item rather than an impression |
| A functional prototype, which impeccable does not generate | `scripts/ux_proto.py` | Not taken from impeccable; added because the two-stage review needs an artefact a person can actually use. Generated entirely from the contract -- the five states as a switch, a form that validates, a dialog that traps focus, a destructive action with a working undo, tabs with arrow keys, a table with tabular figures -- so `ux_check.py` over its output is a positive control on the generator |
...
| The thirty operations (`colorize`, `typeset`, `layout`, `shape`, `animate`, `delight`, `distill`, `bolder`, `quieter`, `overdrive`, `clarify`, `polish`, `craft`, `ios`, `android`, `adapt.native`, `audit.native`, `live`, `live-setup`, `generate`, `visualize`, `document`, `onboard`, `extract`, `adapt`, `doctor`, `routing`, `operate`, `new-work`, `hooks`) | `references/ops/` | Rewritten, and each one ends by naming the detectors that adjudicate its output, so an operation finishes in a status rather than an impression. Three gained a tool where impeccable states a judgement: `palette.py` for `colorize`, `typescale.py` for `typeset`, `ux_delta.sh` for `live`. `audit.native`'s five 0–4 dimension scores were deliberately **not** carried — a composite lets a 2 in accessibility average against a 4 in performance, and the 2 is somebody unable to use the app |

### Still not taken

One thing remains impeccable's, and it is narrower than it was:

| Not taken | Why |
|---|---|
| `live` **variant generation** -- an AI-generated HTML+CSS variant hot-swapped into the running page through the dev server's HMR, then accepted or discarded in the browser, with a manual-edit buffer that commits browser edits back to source | deuxui now takes everything around this and deliberately stops at the swap. `ux_review.py` hosts the real interface -- a file served, a URL proxied, the selection script injected and the frame-blocking headers dropped -- so a person clicks any element and says what is wrong in their own words, and it lands as a work item naming that element. `ux_proto.py` generates the thing they are using. What deuxui does not do is put a generated variant into the DOM: the change belongs in the source, where the dev server's own reload shows it and `ux_delta.sh` measures the delta. That is a position, not an omission -- a variant that exists only in the page has to be committed back afterwards, and impeccable's `live-manual-edits-buffer`, `live-commit-manual-edits` and per-framework adapters are exactly the machinery that round trip needs. It is also a Node subsystem, and deuxui ships python3 plus pyyaml with no build step |

Three things were taken and then deliberately changed, which is different from not
taking them:
...
- The single approval point. deuxui splits it in two, because a wireframe and a
  working prototype answer different questions and only the second one can
  establish that the thing works. `build` is gated on the second (`GOV-014`), and
  "request changes" is a first-class outcome that is explicitly not an approval.

impeccable's Rust detection engine was **not** ported. deuxui ships as a skill directory
...
deuxui **does** now carry that material, by way of impeccable: the `IOS-*` and `AND-*`
...
What deuxui adds on top is the verdict: each guidance bullet is a rule with a severity
