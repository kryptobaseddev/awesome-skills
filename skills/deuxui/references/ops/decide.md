...
`.deuxui/decisions/DEC-NNN.yaml`, hashed against exactly what was shown.

impeccable's `serve-question` is the ancestor; what deuxui adds is that the
record is admissible — see NOTICE.md.

## Serve it

```bash
python3 scripts/ux_question.py ask .deuxui/comps/<surface>.comps.yaml
python3 scripts/ux_question.py ask question.yaml --port 8787 --timeout 1800
```

The page is themed from `.deuxui/design.contract.yaml` — the project's own
...
inheriting a flat project scale would make deuxui's surface fail deuxui's craft
