...
"""R-AXE -- run axe-core against the live page.
  ...
  ux_axe.py --out .deuxui/reports/runtime --route /
  ux_axe.py --no-fetch      # only use a copy already on this machine

axe-core covers rule families deuxui's own probes do not reproduce: ARIA
attribute validity, role-required-children, duplicate ids in active contexts, the
long tail of name-computation cases. deuxui's probes cover things axe does not
...
CACHE = Path(tempfile.gettempdir()) / f"deuxui-axe-{VERSION}.js"

# The rule families deuxui already measures directly. Reporting them twice makes
...
def source(no_fetch=False, root: Path | None = None) -> tuple[str, str]:
    ...
    for url in CDNS:
        try:
            req = urllib.request.Request(url, headers={"User-Agent": "deuxui"})
