...
`.deuxui/decisions/DEC-NNN.yaml`, hashed against exactly what was shown, and the
phase gate reads it.
...
# a project's flat scale would make deuxui's own surface fail deuxui's own craft
...
<p class="meta">served by deuxui &middot; contract {html.escape(str(q.get('contract_sha') or 'none'))}
...
def record(form: dict, q: dict) -> Path:
    ...
    rec = {
        ...
        "shown": [{"id": o["id"], "title": o["title"], "image": o["image"],
                  ...
                  for o in q["options"]],
        "source": q["source"],
        "recorded_by": "deuxui/scripts/ux_question.py (served page, human submitted)",
    }
    DECISIONS.mkdir(parents=True, exist_ok=True)
