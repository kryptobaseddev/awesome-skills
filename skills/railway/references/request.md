...
Official documentation, community endpoints, and GraphQL operations for things the CLI doesn't expose.
...
- **Direct doc pages**: `https://docs.railway.com/<path>` (append `.md` for markdown)
...
Filter by topic with `topic` parameter (`"questions"`, `"feedback"`, `"community"`, `"billing"`):
...
Bulk search alternative — all public threads with full content:
...
Full API schema: https://docs.railway.com/api/llms-docs.md
...
The CLI can create services (`railway add`) but cannot rename them or change icons:
...
After creating via GraphQL, configure with a JSON config patch including `isCreated: true`.

## Webhooks

Railway supports webhooks for external service integration:

```bash
scripts/railway-api.sh \
  'mutation createWebhook($input: WebhookCreateInput!) {
    webhookCreate(input: $input) { id url }
  }' \
  '{"input":{"projectId":"<project-id>","url":"https://example.com/webhook"}}'
```

Webhooks notify external services of Railway events (deployments, status changes).
...
Valid `MetricTag` values for `groupBy`: `SERVICE_ID`, `DEPLOYMENT_ID`, `DEPLOYMENT_INSTANCE_ID`, `REGION`.
...
For deploying into a specific environment, use the two-step GraphQL flow:
