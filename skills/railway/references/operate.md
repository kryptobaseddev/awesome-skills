...
Check health, read logs, query metrics, troubleshoot failures, and recover.
...
Filter syntax: text search (`"error message"`), attribute filters (`@level:error`, `@level:warn`), boolean operators (`AND`, `OR`, `-` for negation). Full syntax: https://docs.railway.com/guides/logs
...
Run arbitrary commands against deployed services:

```bash
railway ssh --service <service>
```

Useful for debugging runtime state, inspecting files on disk, checking environment variables, or running one-off database commands.
...
Omit `serviceId` and add `"groupBy": ["SERVICE_ID"]` to query all services at once. Get IDs from `railway status --json`.

### Metrics for scaled services

Use `groupBy` to break down by replica or region:
...
scripts/railway-api.sh \
  ...
  '{"environmentId":"<env-id>","serviceId":"<service-id>","startDate":"2026-02-19T00:00:00Z","groupBy":["DEPLOYMENT_INSTANCE_ID"],"measurements":["CPU_USAGE","MEMORY_USAGE_GB"]}'
```

Valid `MetricTag` values: `SERVICE_ID`, `DEPLOYMENT_ID`, `DEPLOYMENT_INSTANCE_ID`, `REGION`.

Optional parameters: `endDate` (defaults to now), `sampleRateSeconds`, `averagingWindowSeconds`.

Pro plan retains up to 7 days of metrics data.
...
- **Out of memory**: check metrics for memory spikes, consider optimizing or requesting higher limits
...
Deployment stuck in `DEPLOYING` or fails after timeout:
...
Common causes:
- Service takes longer to start than `healthcheckTimeout` — increase the timeout
- Health endpoint returns non-200 — check the endpoint logic
- Service crashes during startup — check runtime logs for errors
...
- **No deployments found**: the service exists but has never deployed
- **Metrics query returns empty**: verify IDs from `railway status --json` and check the time window
- **Config patch type error**: check the typed paths in [configure.md](configure.md)
- **Logs streaming forever**: always use `--lines`, `--since`, or `--until` for bounded queries
