...
deluxui's claim has always been that the contract is declared BEFORE the code it
...
SKIP = {"node_modules", ".git", "dist", "build", ".next", ".svelte-kit", "out",
        "coverage", ".venv", "venv", "__pycache__", ".turbo", ".cache", ".deluxui",
        "target", "Pods", ".gradle"}
MIN_REASON = 40
...
def process_checks(st: dict) -> dict:
    ...
    out = {}
    if not st:
        why = ("No .deluxui/phase.yaml. The order the work happened in was never "
...
def gate_write(path: str, st: dict) -> tuple[int, str]:
    ...
    if not st:
        return 0, ("No phase state, so nothing is being gated. `ux_phase.py init` "
                   "turns this on; until then deluxui reports the order of work as "
...
def cmd_status(a) -> int:
    ...
    if not st:
        ...
        sys.stderr.write("No .deluxui/phase.yaml. Nothing is gated and the order of "
