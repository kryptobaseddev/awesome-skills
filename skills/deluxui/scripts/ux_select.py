...
Each answer becomes `.deluxui/requests/REQ-NNN.yaml`, which names an operation and
...
has to be committed back later, and that round trip is where an edit gets lost.
    ...
    ux_select.py watch [--timeout 1800] [--out .deluxui/requests]
    ux_select.py list
    ux_select.py clear
...
(() => {
  ...
  badge.textContent = 'deluxui · click anything · Esc to stop';
  ...
  function openPanel(info) {
    ...
    panel.querySelectorAll('button.act').forEach(b => b.onclick = () => {
      ...
      badge.textContent = `deluxui · ${S.queue.length} captured · Esc to stop`;
      closePanel();
    });
  ...
  addEventListener('keydown', e => {
    if (e.key === 'Escape') {
      ...
      badge.textContent = `deluxui · stopped · ${S.queue.length} captured`;
    }
  }, true);
  return "installed";
})()
...
def write(rec: dict) -> Path:
    ...
    out["id"] = rid
    out["recorded_at"] = now()
    out["recorded_by"] = ("deluxui/scripts/ux_select.py -- a person pointed at this "
