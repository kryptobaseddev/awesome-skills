...
deluxui has always claimed that the visual contract is declared **before** the code
...
impeccable's `build-phase` is the ancestor; what deluxui adds is that the order is
...
With no `.deluxui/phase.yaml`, nothing is refused and the process detectors report
