...
bash scripts/ux_browser.sh <url> \
  ...
  --out .deluxui/reports/runtime

# record the manual results a machine cannot produce
$EDITOR .deluxui/reports/manual.yaml      # see verification/manual-checks.md
```
...
A deviation goes in `.deluxui/exceptions.yaml` using `assets/templates/exception.md`:
