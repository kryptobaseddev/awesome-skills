...
| Not taken | Why |
|---|---|
| `live` **variant generation** -- an AI-generated HTML+CSS variant hot-swapped into the running page through the dev server's HMR, then accepted or discarded in the browser, with a manual-edit buffer that commits browser edits back to source | deluxui now takes everything around this and deliberately stops at the swap. `ux_review.py` hosts the real interface -- a file served, a URL proxied, the selection script injected and the frame-blocking headers dropped -- so a person clicks any element and says what is wrong in their own words, and it lands as a work item naming that element. `ux_proto.py` generates the thing they are using. What deluxui does not do is put a generated variant into the DOM: the change belongs in the source, where the dev server's own reload shows it and `ux_live.sh` measures the delta. That is a position, not an omission -- a variant that exists only in the page has to be committed back afterwards, and impeccable's `live-manual-edits-buffer`, `live-commit-manual-edits` and per-framework adapters are exactly the machinery that round trip needs. It is also a Node subsystem, and deluxui ships python3 plus pyyaml with no build step |
...
- The single approval point. deluxui splits it in two, because a wireframe and a
