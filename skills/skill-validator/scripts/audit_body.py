...
def audit_body(skill_path):
    ...
    # ── Placeholder scan — case-sensitive, strips code spans ────────────
    # Documentation that mentions placeholder tokens in code formatting
    # (`TODO`, ```...```) is not a real placeholder. Strip those before scan.
    placeholder_scan_text = re.sub(r"```[\s\S]*?```", "", body)
    placeholder_scan_text = re.sub(r"`[^`\n]+`", "", placeholder_scan_text)
    ...
    for pattern, label in placeholder_patterns:
        matches = re.findall(pattern, placeholder_scan_text)
        if matches:
